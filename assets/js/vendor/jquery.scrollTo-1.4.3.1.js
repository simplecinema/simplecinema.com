...
(function ($) {
  ...
  $.fn._scrollable = function () {
    return this.map(function () {
      ...
      return /webkit/i.test(navigator.userAgent) || doc.compatMode ==
        'BackCompat' ?
        doc.body :
        doc.documentElement;
    });
    ...
    return this._scrollable().each(function () {
      ...
      $.each(settings.axis.split(''), function (i, axis) {
        ...
        if (toff) { // jQuery / DOMElement
          ...
          if (settings.margin) {
            attr[key] -= parseInt(targ.css('margin' + Pos)) ||
              0;
            attr[key] -= parseInt(targ.css('border' + Pos +
              'Width')) || 0;
          }
  ...
  $scrollTo.max = function (elem, axis) {
    ...
    return Math.max(html[scroll], body[scroll]) - Math.min(html[
      size], body[size]);
  };
